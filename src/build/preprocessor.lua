...
local function load( self, data )
	...
	elseif fs.exists( "sheets/build" .. "/module/" .. data:gsub( "%.", "/" ) .. ".lua" ) then
		loadModule( self, "sheets/build" .. "/module/" .. data:gsub( "%.", "/" ) .. ".lua", data )
	else
...
function preprocessor:push( str )
	...
	while str do
		line = str:match "^(.-)\n" or str
		str = str:match "^.-\n(.+)"
		if line and line:find "^%s*%-?%-?%s*@%s*%w" then
			instructions[#instructions + 1] = { line = #lines + 1, instruction = line:match "^%s*%-?%-?%s*@%s*(%w+)":lower(), data = line:gsub( "^%s*%-?%-?%s*@%s*%w+%s*", "" ) }
			line = ""
		end
