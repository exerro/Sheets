...
components = {}
 ...
 -- @define WITHOUT(name) (function(data)return{type="independency",component='name',data=data}end)
	...
	for i = 1, #data do
		if data[i].type == "property" or data[i].type == "getter" or data[i].type == "setter" then
			if res[data[i].property] then
				...
				if type( data[i].options ) == "table" then
					for n = 1, #data[i].options do
						if type( data[i].options[n] ) == "table" and data[i].options[n].type == "dependency" and lookup[components[data[i].options[n].component]] then
							for k, v in pairs( data[i].options[n] ) do
								data[i].options[k] = v
							end
						elseif type( data[i].options[n] ) == "table" and data[i].options[n].type == "independency" and not lookup[components[data[i].options[n].component]] then
							for k, v in pairs( data[i].options[n] ) do
								data[i].options[k] = v
							end
						end
		...
		elseif data[i].type == "independency" then
			if not lookup[components[data[i].component]] then
				add_data_t( res, changed, data[i].data, lookup, src )
			end
		end
	end
